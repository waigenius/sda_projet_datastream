...
def task_consume(**_) -> List[Dict[str, Any]]:
    ...
    TEST_FILE = Path("/exports/preview/preview-20251029T201952Z.json")  # <-- adapte ce nom à ton fichier
...
def _to_iso_z(ts_val):
    """Normalise un timestamp en ISO8601 Z. Accepte ISO, epoch (s/ms)."""
    if not ts_val:
        return None
    # déjà ISO '...Z' ou '...+00:00'
    if isinstance(ts_val, str):
        try:
            # Python 3.11+ tolère 'Z' via replace
...
def task_transform(ti, **_) -> List[Dict[str, Any]]:
    ...
    for d in raw_docs:
        try:
            ...
            # ✅ préserver le timestamp d'entrée, sinon fallback à now UTC
            incoming_ts = d.get("agent_timestamp") or d.get("agentTimestamp") or d.get("timestamp")
            norm_ts = _to_iso_z(incoming_ts)
            if not norm_ts:
                norm_ts = datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%SZ")
            d["agent_timestamp"] = norm_ts
    ...
    # ===== Version présentation (inchangée, elle reprend agent_timestamp des docs) =====
    simplified_list = []
    for d in out:
        simp = {
            ...
            "agent_timestamp": d.get("agent_timestamp", ""),  # ✅ conserve l'événement
        }
        simplified_list.append(simp)

    # Écriture JSON locale (présentation)
    try:
