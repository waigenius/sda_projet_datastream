...
CREATE OR REPLACE TABLE 
  `votre-projet-gcp.taxi_streaming_project.trajets_avec_coordonnees` AS
SELECT 
  *,
...
FROM `votre-projet-gcp.taxi_streaming_project.trajets_streaming_external`
WHERE locationClient IS NOT NULL AND prix_travel IS NOT NULL;

CREATE OR REPLACE TABLE 
  `votre-projet-gcp.taxi_streaming_project.trajets_avec_clusters` AS
SELECT 
  t.*,
  p.CENTROID_ID as cluster_id
FROM `votre-projet-gcp.taxi_streaming_project.trajets_avec_coordonnees` t
LEFT JOIN (
  SELECT * FROM ML.PREDICT(
    MODEL `votre-projet-gcp.taxi_streaming_project.taxi_geocluster_model`,
    (SELECT nomClient, client_lon AS lon, client_lat AS lat
     FROM `votre-projet-gcp.taxi_streaming_project.trajets_avec_coordonnees`)
  )
...
FROM `votre-projet-gcp.taxi_streaming_project.trajets_avec_clusters`
